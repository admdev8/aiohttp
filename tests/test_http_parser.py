...
"""
    ...
    assert list(msg.headers.items()) == [("test", "line continue"),
                                         ("test2", "data")]
    assert msg.raw_headers == ((b"test", b"line continue"), (b"test2",
                                                             b"data"))
...
def test_max_header_field_size(parser, size) -> None:
    ...
    match = "400, message='Got more than 8190 bytes \\({}\\) when reading".format(
        size)
...
def test_http_request_max_status_line_under_limit(parser) -> None:
    path = b"t" * (8190 - 5)
    messages, upgraded, tail = parser.feed_data(b"GET /path" + path +
                                                b" HTTP/1.1\r\n\r\n")
    msg = messages[0][0]
...
def test_http_response_parser_status_line_under_limit(response) -> None:
    reason = b"O" * 8190
    messages, upgraded, tail = response.feed_data(b"HTTP/1.1 200 " + reason +
                                                  b"\r\n\r\n")
    msg = messages[0][0]
    assert msg.version == (1, 1)
    assert msg.code == 200
